...
local instance holomorphicLineBundleExtensionTopology : TopologicalSpace (ComplexPoint X s) :=
...
def transitionValue (i j x : ComplexPoint X s) : ℂ := by
  classical
...
theorem contMDiffOn_transitionValue (i j : ComplexPoint X s) :
...
theorem transitionValue_self (i x : ComplexPoint X s) (hx : x ∈ E.localLifts.opens i) :
...
theorem transitionValue_mul (i j k x : ComplexPoint X s)
...
def lineBundleCore : VectorBundleCore ℂ (ComplexPoint X s) ℂ (ComplexPoint X s) where
...
theorem lineBundleCore_finrank (x : ComplexPoint X s) :
