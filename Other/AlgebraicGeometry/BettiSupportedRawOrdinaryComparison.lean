...
theorem rationalCohomologyAmbient_forgetSupport_rawSingularCone
      ...
      HomologicalComplex.homologyMap
        ...
        ((HomologicalComplex.homologyFunctor AddCommGrpCat (.up ℤ) 0).shiftMap
          ...
          ((inv (HomologicalComplex.homologyMap
            (globalRawSupportConeToGlobalNaturalSingularCone X Z hZ) ((n : ℤ) - 1)))
            (HomologicalComplex.homologyMap
              (globalSectionsNaturalSingularConeIsoMappingCone X Z hZ).hom ((n : ℤ) - 1)
  ...
  let r := globalRawSingularRestrictionInt ℚ (TopCat.of (ComplexPoint X))
    Zᶜ
  let y := rationalSupportHypercohomologyAddEquivNaturalSingularConeGlobalSections X Z hZ (n : ℤ) a
  ...
  have he := CochainComplex.mappingCone.mapHomologicalComplexIso_homology_connecting
    b Γ ((n : ℤ) - 1) (n : ℤ) (by omega)
  have hc : HomologicalComplex.homologyMap c ((n : ℤ) - 1) ≫
      H.shiftMap (CochainComplex.mappingCone.triangle (F.map b)).mor₃ ((n : ℤ) - 1) (n : ℤ) (by omega) ≫
        HomologicalComplex.homologyMap (F.map u) (n : ℤ) =
      H.shiftMap (CochainComplex.mappingCone.triangle r).mor₃ ((n : ℤ) - 1) (n : ℤ) (by omega) ≫
        HomologicalComplex.homologyMap
          (globalRawToSingularSheafInt X ≫ F.map u) (n : ℤ) := by
    have hh := congrArg (fun f => H.shiftMap f ((n : ℤ) - 1) (n : ℤ) (show (1 : ℤ) + ((n : ℤ) - 1) = (n : ℤ) by omega))
    ...
    exact hh
  have hc' := congrArg
    (fun f => inv (HomologicalComplex.homologyMap c ((n : ℤ) - 1)) ≫ f) hc
  ...
  change H.shiftMap (ShiftedHom.map
    ...
    ((n : ℤ) - 1) (n : ℤ) (by omega) y = _
  rw [hmap, Functor.shiftMap_comp]
  erw [← he]
  change (HomologicalComplex.homologyMap e.hom ((n : ℤ) - 1) ≫
    H.shiftMap (CochainComplex.mappingCone.triangle (F.map b)).mor₃
      ((n : ℤ) - 1) (n : ℤ) (by omega) ≫ HomologicalComplex.homologyMap (F.map u) (n : ℤ)) y = _
  exact ConcreteCategory.congr_hom (congrArg
    (fun f => HomologicalComplex.homologyMap e.hom ((n : ℤ) - 1) ≫ f) hc') y
