...
def ambientRationalInjectiveComplex :
  ...
  (TopCat.Sheaf.ambientConstantInjectiveResolution
    (TopCat.of (ComplexPoint X structureMap)) (AddCommGrpCat.of ℚ)).cocomplex.extend
...
def ambientRationalInjectiveAugmentation :
  ...
  HomologicalComplex.extendMap
    (TopCat.Sheaf.ambientConstantInjectiveResolution
      (TopCat.of (ComplexPoint X structureMap)) (AddCommGrpCat.of ℚ)).ι
    ComplexShape.embeddingUpNat

instance ambientRationalInjectiveAugmentation_quasiIso :
    QuasiIso (ambientRationalInjectiveAugmentation structureMap) := by
  let I := TopCat.Sheaf.ambientConstantInjectiveResolution
    (TopCat.of (ComplexPoint X structureMap)) (AddCommGrpCat.of ℚ)
...
instance ambientRationalInjectiveComplex_injective (q : ℤ) :
    Injective ((ambientRationalInjectiveComplex structureMap).X q) :=
  CochainComplex.injective_extend_nat _
    (TopCat.Sheaf.ambientConstantInjectiveResolution
      (TopCat.of (ComplexPoint X structureMap)) (AddCommGrpCat.of ℚ)).injective q

instance ambientRationalInjectiveComplex_isStrictlyGE :
    (ambientRationalInjectiveComplex structureMap).IsStrictlyGE 0 := by
...
lemma ambientRationalAugmentation_comp_restriction
    ...
    ambientRationalInjectiveAugmentation structureMap ≫
      ambientRationalInjectiveRestriction structureMap Z hZ =
        rationalRestrictionComplexInt structureMap Z := by
  change (ComplexShape.embeddingUpNat.extendFunctor
    (AnalyticAdditiveSheaf structureMap)).map _ ≫
      (ComplexShape.embeddingUpNat.extendFunctor
        (AnalyticAdditiveSheaf structureMap)).map _ = _
  rw [← Functor.map_comp]
  exact congrArg
    ((ComplexShape.embeddingUpNat.extendFunctor (AnalyticAdditiveSheaf structureMap)).map)
...
def rationalSupportConeToAmbientInjectiveCone
  ...
  CochainComplex.mappingCone.map _ _
    (ambientRationalInjectiveAugmentation structureMap) (𝟙 _)
    (by simpa using (ambientRationalAugmentation_comp_restriction structureMap Z hZ).symm)

instance rationalSupportConeToAmbientInjectiveCone_quasiIso
    (Z : Set (ComplexPoint X structureMap)) (hZ : IsClosed Z) :
    QuasiIso (rationalSupportConeToAmbientInjectiveCone structureMap Z hZ) :=
...
lemma rationalSupportConeToAmbientInjectiveCone_connecting
    (Z : Set (ComplexPoint X structureMap)) (hZ : IsClosed Z) :
    rationalSupportConeToAmbientInjectiveCone structureMap Z hZ ≫
      (CochainComplex.mappingCone.triangle
        (ambientRationalInjectiveRestriction structureMap Z hZ)).mor₃ =
    (CochainComplex.mappingCone.triangle (rationalRestrictionComplexInt structureMap Z)).mor₃ ≫
      (ambientRationalInjectiveAugmentation structureMap)⟦(1 : ℤ)⟧' :=
  (CochainComplex.mappingCone.triangleMap _ _
    (ambientRationalInjectiveAugmentation structureMap) (𝟙 _)
    (by simpa using (ambientRationalAugmentation_comp_restriction structureMap Z hZ).symm)).comm₃.symm

/-- The replacement cone is bounded below. -/
instance ambientRationalInjectiveCone_isStrictlyGE
    (Z : Set (ComplexPoint X structureMap)) (hZ : IsClosed Z) :
    (CochainComplex.mappingCone
      (ambientRationalInjectiveRestriction structureMap Z hZ)).IsStrictlyGE (-1) := by
  let : (derivedPushforwardComplementConstantRationalComplexInt structureMap Z).IsStrictlyGE 0 := by
...
lemma ambientRationalInjectiveCone_isFlasque
    ...
    ((CochainComplex.mappingCone
      (ambientRationalInjectiveRestriction structureMap Z hZ)).X q).IsFlasque := by
...
def rationalSupportAddEquivAmbientInjectiveConeGlobalSections
    ...
    RationalCohomologyWithSupport structureMap Z n ≃+
      (TopCat.Sheaf.globalSectionsComplexInt
        ...
        (CochainComplex.mappingCone
          (ambientRationalInjectiveRestriction structureMap Z hZ))).homology (n - 1) := by
  let e : RationalCohomologyWithSupport structureMap Z n ≃+
      Hypercohomology structureMap (CochainComplex.mappingCone
        (ambientRationalInjectiveRestriction structureMap Z hZ)) (n - 1) :=
...
def ambientRationalOpenResolutionComparison
    ...
    ((TopCat.Sheaf.openRestrictionPushforward
      (TopCat.of (ComplexPoint X structureMap)) ⟨Zᶜ, hZ.isOpen_compl⟩).mapHomologicalComplex
      (.up ℤ)).obj (ambientRationalInjectiveComplex structureMap) ⟶
        derivedPushforwardComplementConstantRationalComplexInt structureMap Z :=
  (HomologicalComplex.mapExtendIso
    (TopCat.Sheaf.openRestrictionPushforward
      (TopCat.of (ComplexPoint X structureMap)) ⟨Zᶜ, hZ.isOpen_compl⟩)
    (TopCat.Sheaf.ambientConstantInjectiveResolution
      (TopCat.of (ComplexPoint X structureMap)) (AddCommGrpCat.of ℚ)).cocomplex
      ComplexShape.embeddingUpNat).hom ≫
    HomologicalComplex.extendMap
      (((TopCat.Sheaf.pushforward AddCommGrpCat
        (analyticComplementInclusion structureMap Z)).mapHomologicalComplex (.up ℕ)).map
...
lemma actualRestriction_comp_openResolutionComparison
    ...
    (TopCat.Sheaf.supportRestrictionComplexShortComplex
      (TopCat.of (ComplexPoint X structureMap)) ⟨Zᶜ, hZ.isOpen_compl⟩
      (ambientRationalInjectiveComplex structureMap)).g ≫
        ambientRationalOpenResolutionComparison structureMap Z hZ =
      ambientRationalInjectiveRestriction structureMap Z hZ := by
  ...
  exact (ComplexShape.embeddingUpNat.extendFunctor (AnalyticAdditiveSheaf structureMap)).map_comp _ _
    |>.symm
...
def globalAmbientRationalOpenResolutionComparison
  ...
  ((TopCat.Sheaf.IsFlasque.BoundedBelowComplex.globalSectionsFunctor
    (TopCat.of (ComplexPoint X structureMap))).mapHomologicalComplex (.up ℤ)).map
      (ambientRationalOpenResolutionComparison structureMap Z hZ)

set_option backward.isDefEq.respectTransparency false in
instance globalAmbientRationalOpenResolutionComparison_quasiIso
    (Z : Set (ComplexPoint X structureMap)) (hZ : IsClosed Z) :
    QuasiIso (globalAmbientRationalOpenResolutionComparison structureMap Z hZ) := by
...
def actualSupportConeToAmbientInjectiveGlobalCone
  ...
  CochainComplex.mappingCone.map _ _ (𝟙 _)
    (globalAmbientRationalOpenResolutionComparison structureMap Z hZ) (by
...
instance actualSupportConeToAmbientInjectiveGlobalCone_quasiIso
    (Z : Set (ComplexPoint X structureMap)) (hZ : IsClosed Z) :
    QuasiIso (actualSupportConeToAmbientInjectiveGlobalCone structureMap Z hZ) :=
  CochainComplex.mappingCone.quasiIso_map_of_quasiIso _ _ _ _ _
...
lemma actualSupportConeToAmbientInjectiveGlobalCone_connecting
    (Z : Set (ComplexPoint X structureMap)) (hZ : IsClosed Z) :
    actualSupportConeToAmbientInjectiveGlobalCone structureMap Z hZ ≫
      (CochainComplex.mappingCone.triangle
        (((TopCat.Sheaf.IsFlasque.BoundedBelowComplex.globalSectionsFunctor
          (TopCat.of (ComplexPoint X structureMap))).mapHomologicalComplex (.up ℤ)).map
            (ambientRationalInjectiveRestriction structureMap Z hZ))).mor₃ =
    (CochainComplex.mappingCone.triangle
      (TopCat.Sheaf.supportRestrictionSectionsComplexShortComplex
        (TopCat.of (ComplexPoint X structureMap)) ⟨Zᶜ, hZ.isOpen_compl⟩ ⊤
        (ambientRationalInjectiveComplex structureMap)).g).mor₃ := by
  have h := (CochainComplex.mappingCone.triangleMap
    ...
    (((TopCat.Sheaf.IsFlasque.BoundedBelowComplex.globalSectionsFunctor
      (TopCat.of (ComplexPoint X structureMap))).mapHomologicalComplex (.up ℤ)).map
        (ambientRationalInjectiveRestriction structureMap Z hZ)) (𝟙 _)
    (globalAmbientRationalOpenResolutionComparison structureMap Z hZ)
    (show _ = _ from by
      let Γ := (TopCat.Sheaf.IsFlasque.BoundedBelowComplex.globalSectionsFunctor
        (TopCat.of (ComplexPoint X structureMap))).mapHomologicalComplex (.up ℤ)
      change Γ.map _ ≫ Γ.map _ = 𝟙 _ ≫ Γ.map _
  ...
  exact (congrArg (fun f =>
    (CochainComplex.mappingCone.triangle
      (TopCat.Sheaf.supportRestrictionSectionsComplexShortComplex
        (TopCat.of (ComplexPoint X structureMap)) ⟨Zᶜ, hZ.isOpen_compl⟩ ⊤
        (ambientRationalInjectiveComplex structureMap)).g).mor₃ ≫ f)
...
def rationalSupportAddEquivSupportedInjectiveHomology
    ...
    RationalCohomologyWithSupport structureMap Z n ≃+
      (TopCat.Sheaf.supportRestrictionSectionsComplexShortComplex
        (TopCat.of (ComplexPoint X structureMap)) ⟨Zᶜ, hZ.isOpen_compl⟩ ⊤
        (ambientRationalInjectiveComplex structureMap)).X₁.homology n := by
  let Y := TopCat.of (ComplexPoint X structureMap)
  let U : Opens Y := ⟨Zᶜ, hZ.isOpen_compl⟩
  let Γ := TopCat.Sheaf.IsFlasque.BoundedBelowComplex.globalSectionsFunctor Y
  let S := TopCat.Sheaf.supportRestrictionSectionsComplexShortComplex Y U ⊤
    (ambientRationalInjectiveComplex structureMap)
  let e₁ := rationalSupportAddEquivAmbientInjectiveConeGlobalSections structureMap Z hZ n
  let e₂ := HomologicalComplex.homologyMapIso
    (CochainComplex.mappingCone.mapHomologicalComplexIso
      (ambientRationalInjectiveRestriction structureMap Z hZ) Γ) (n - 1)
  let e₃ := (asIso (HomologicalComplex.homologyMap
    (actualSupportConeToAmbientInjectiveGlobalCone structureMap Z hZ) (n - 1))).symm
