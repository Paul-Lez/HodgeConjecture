...
/-!
# Explicit paths on the elliptic cubic

This file constructs paths on the cubic `Y²Z = X³ - XZ²` from the point at infinity to the
finite branch point `[0:0:1]`. The paths are first defined by homogeneous coordinate vectors,
then lifted through the closed projective embedding. The resulting connected-component
statements are also exported on the top-open subtype used by global holomorphic functions.
-/
...
abbrev curveComplexSpace : Over (Spec (CommRingCat.of ℂ)) := Over.mk curveToBase

abbrev curvePlaneComplexSpace : Over (Spec (CommRingCat.of ℂ)) :=
  Over.mk (ProjectiveSpace.toBase (Fin 3) base)

def curvePointProjectiveEmbedding : curveComplexSpace ⟶ curvePlaneComplexSpace := Over.homMk curveToPlane rfl

instance : IsClosedImmersion curvePointProjectiveEmbedding.left :=
...
lemma curvePointProjectiveEmbedding_apply (P : Fin 3 → ℂ) (hP : P ≠ 0)
    (heq : equation.toProjective.Equation P) :
    Point.map curvePointProjectiveEmbedding (curvePoint P hP heq) = vectorToComplexPoint P hP := by
  simpa only [curvePointProjectiveEmbedding] using curvePoint_map_vectorToComplexPoint P hP heq
...
lemma continuous_curvePoint_family {T : Type*} [TopologicalSpace T]
    ...
    @Continuous T (ComplexPoint curveComplexSpace) inferInstance Point.analyticTopology
      (fun t ↦ curvePoint (P t) (hP t) (heq t)) := by
  apply (@IsInducing.continuous_iff T (ComplexPoint curveComplexSpace)
    (ComplexPoint curvePlaneComplexSpace) (fun t ↦ curvePoint (P t) (hP t) (heq t))
    (Point.map curvePointProjectiveEmbedding) Point.analyticTopology inferInstance Point.analyticTopology
    (isInducing_map_of_closedImmersion curvePointProjectiveEmbedding)).mpr
  rw [show (Point.map curvePointProjectiveEmbedding ∘ fun t ↦ curvePoint (P t) (hP t) (heq t)) =
      fun t ↦ vectorToComplexPoint (P t) (hP t) by
    funext t
    exact curvePointProjectiveEmbedding_apply (P t) (hP t) (heq t)]
  change @Continuous T
...
def branchPointOne : ComplexPoint curveComplexSpace :=
  curvePoint ![1, 0, 1]
    (by intro h; have := congrFun h 2; simp at this)
    (by simp [equation, WeierstrassCurve.Projective.equation_iff])

def branchPointZero : ComplexPoint curveComplexSpace :=
  curvePoint ![0, 0, 1]
    (by intro h; have := congrFun h 2; simp at this)
    (by simp [equation, WeierstrassCurve.Projective.equation_iff])
...
def infinityToOnePath (t : Set.Icc (0 : ℝ) 1) : ComplexPoint curveComplexSpace :=
...
def zeroToOnePath (s : Set.Icc (0 : ℝ) 1) : ComplexPoint curveComplexSpace :=
...
def curvePointToTopOpen (x : ComplexPoint curveComplexSpace) :
    (⊤ : Opens (TopCat.of (ComplexPoint curveComplexSpace))) := ⟨x, trivial⟩

lemma continuous_curvePointToTopOpen : Continuous curvePointToTopOpen :=
  continuous_id.subtype_mk _

theorem branchPointZeroTop_mem_connectedComponent_infinityTop :
    curvePointToTopOpen branchPointZero ∈ connectedComponent (curvePointToTopOpen infinity) :=
