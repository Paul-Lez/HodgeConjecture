...
private theorem contMDiff_complexPoint_map
    (X Y : Over (Spec ↧ℂ)) (f : X ⟶ Y) (d e : ℕ)
    [SmoothOfRelativeDimension d X.hom]
    [SmoothOfRelativeDimension e Y.hom] :
    ContMDiff 𝓘(ℂ, Fin d → ℂ) 𝓘(ℂ, Fin e → ℂ) ω
      (Point.map f) := by
  intro z
  let z' := Point.map f z
  have hz : z ∈ (ComplexPoint.localChart X d z).source :=
    ComplexPoint.mem_localChart_source X d z
  have hz' : z' ∈ (ComplexPoint.localChart Y e z').source :=
    ComplexPoint.mem_localChart_source Y e z'
  rw [contMDiffAt_iff_of_mem_source
    (I := 𝓘(ℂ, Fin d → ℂ)) (I' := 𝓘(ℂ, Fin e → ℂ))
    (x := z) (y := z') hz hz']
  refine ⟨(Point.continuous_map f).continuousAt, ?_⟩
  have hw : ComplexPoint.localChart X d z z ∈
      (ComplexPoint.localChart X d z).target :=
    (ComplexPoint.localChart X d z).map_source hz
  have hmap : Point.map f
      ((ComplexPoint.localChart X d z).symm (ComplexPoint.localChart X d z z)) ∈
        (ComplexPoint.localChart Y e z').source := by
    rw [(ComplexPoint.localChart X d z).left_inv hz]
    exact hz'
  have ha := ComplexPoint.analyticAt_localChart_symm_map X Y f d e z hw hmap
  have hsourceChart : chartAt (Fin d → ℂ) z = ComplexPoint.localChart X d z := rfl
  have htargetChart : chartAt (Fin e → ℂ) z' = ComplexPoint.localChart Y e z' := rfl
  simpa only [extChartAt_coe, extChartAt_coe_symm,
    ...
    id_eq, Set.range_id, hsourceChart, htargetChart, z'] using
...
theorem contMDiff_projectivizationToComplexPoint_chartPoint (N : ℕ) (i : Fin (N + 1)) :
  ...
  have hmap := contMDiff_complexPoint_map
