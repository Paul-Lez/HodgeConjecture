...
local instance holomorphicLineBundleSectionsTopology : TopologicalSpace (ComplexPoint X s) :=
  Point.analyticTopology

variable (E : HolomorphicUnitExtension s d)

/-- Fiber coordinates extended by zero outside an open set. -/
def extendSection (U : Opens (TopCat.of (ComplexPoint X s))) (f : U → ℂ)
    (x : ComplexPoint X s) : E.lineBundleCore.Fiber x := by
  classical
  exact if hx : x ∈ U then f ⟨x, hx⟩ else 0

/-- A partial section represented as a total-space map on the base. -/
def extendedSectionMap (U : Opens (TopCat.of (ComplexPoint X s))) (f : U → ℂ) :
    ComplexPoint X s → E.lineBundleCore.TotalSpace :=
  fun x => TotalSpace.mk' ℂ x (E.extendSection U f x)

/-- The extended section agrees with the given fiber coordinates on its domain. -/
theorem extendSection_of_mem (U : Opens (TopCat.of (ComplexPoint X s))) (f : U → ℂ)
    (x : ComplexPoint X s) (hx : x ∈ U) : E.extendSection U f x = f ⟨x, hx⟩ :=
  dif_pos hx

/-- The extended section is zero outside its domain. -/
theorem extendSection_of_notMem (U : Opens (TopCat.of (ComplexPoint X s))) (f : U → ℂ)
    (x : ComplexPoint X s) (hx : x ∉ U) : E.extendSection U f x = 0 :=
  dif_neg hx

/-- On the domain, the total-space map has the prescribed fiber coordinate. -/
theorem extendedSectionMap_of_mem (U : Opens (TopCat.of (ComplexPoint X s))) (f : U → ℂ)
    (x : ComplexPoint X s) (hx : x ∈ U) : E.extendedSectionMap U f x =
...
def holomorphicSectionPredicate :
    TopCat.LocalPredicate (fun _ : TopCat.of (ComplexPoint X s) => ℂ) where
...
def sectionAddSubgroup (U : Opens (TopCat.of (ComplexPoint X s))) : AddSubgroup (U → ℂ) where
...
abbrev HolomorphicSections (U : Opens (TopCat.of (ComplexPoint X s))) :=
  ↥(E.sectionAddSubgroup U)

/-- Restriction of holomorphic sections is additive. -/
def sectionRestriction {U V : Opens (TopCat.of (ComplexPoint X s))} (i : U ⟶ V) :
...
def sectionPresheaf : TopCat.Presheaf AddCommGrpCat (TopCat.of (ComplexPoint X s)) where
...
def sectionSheaf : AnalyticAdditiveSheaf s where
