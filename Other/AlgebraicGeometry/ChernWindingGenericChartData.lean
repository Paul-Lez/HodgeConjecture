...
/-!
# Generic winding charts, and the proof of `NormalizesCoclass`

`GenericWindingChartData X c x d q` refines `GeometricWindingChartData` by pinning the analytic
...
From these, `GenericWindingChartData.normalizesCoclass` **proves** obligation (c),
...
local instance chernWindingGenericChartDataTopology :
...
structure GenericWindingChartData (q : ComplexPoint X) where
  ...
  /-- the unit by which the analytified local equation differs from the normal coordinate -/
  normalUnit : C(((flattenedSupportNeighborhood (Fin (d - 1) → ℂ) 1 chart q mem_source :
    Opens (ComplexPoint X)) : Set (ComplexPoint X)), ℂ)
  normalUnit_ne_zero : ∀ y, normalUnit y ≠ 0
  /-- **the analytified local equation is the normal coordinate of the chart, up to a unit** -/
  normal_coordinate : ∀ w, windingUnitFunction X d
    ...
    normalUnit (ChernWinding.flattenedPuncturedInclusion (S := cycleComponentSupport X x)
      (Fin (d - 1) → ℂ) chart q mem_source w) *
...
variable {X c x d} {q : ComplexPoint X} (G : GenericWindingChartData X c x d q)
...
theorem normalizesCoclass (hx : coheight x = ((1 : ℕ) : ℕ∞)) :
  ...
  congr 1
  exact ChernWinding.windingClass_eq_chartNormalProjectionCoclass_of_unit (Fin (d - 1) → ℂ)
    G.chart G.flattens q G.mem_source G.center _ _ G.normalUnit G.normalUnit_ne_zero
    G.normal_coordinate
