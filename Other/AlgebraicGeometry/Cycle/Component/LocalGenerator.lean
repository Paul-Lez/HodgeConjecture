...
/-- The selected smooth source point regarded as a complex point of the smooth locus. -/
def smoothPoint : ComplexPoint (sourceSmoothScheme i) :=
  ComplexPoint.asOpenPoint (Y)
    (sourceSmoothLocus i)
    C.point C.point_mem_smoothLocus

/-- The selected affine source neighborhood, bundled over the complex base. -/
...
def neighborhoodPoint :
  ...
  ComplexPoint.asOpenPoint (sourceSmoothScheme i) C.sourceNeighborhood
    (smoothPoint C) (by
      change (smoothPoint C).underlying ∈ C.sourceNeighborhood
      have hmap : Point.map (ComplexPoint.openInclusion (Y)
          (sourceSmoothLocus i)) (smoothPoint C) =
          C.point :=
        congrArg Subtype.val
          ((ComplexPoint.openEquiv (Y)
            (sourceSmoothLocus i)).apply_symm_apply
              ⟨C.point, C.point_mem_smoothLocus⟩)
      ...
      have hu' : (smoothPoint C).underlying =
          (⟨C.point.underlying, C.point_mem_smoothLocus⟩ :
            (sourceSmoothLocus i).toScheme) := Subtype.ext hu
      rw [hu']
      exact C.point_mem_sourceNeighborhood)
...
lemma neighborhoodToSpecΓ_over :
    C.sourceNeighborhood.toScheme.toSpecΓ ≫
        ComplexPoint.affineSpecStructureMap Γ(C.sourceNeighborhood.toScheme, ⊤) =
      C.neighborhoodStructureMap := by
  let φ : ↧ℂ ⟶ Γ(C.sourceNeighborhood.toScheme, ⊤) :=
...
def neighborhoodToSpecΓIso : C.neighborhoodScheme ≅
    Over.mk (ComplexPoint.affineSpecStructureMap Γ(C.sourceNeighborhood.toScheme, ⊤)) :=
  letI : IsAffine C.sourceNeighborhood.toScheme :=
    C.sourceNeighborhood_isAffine
  letI : IsIso C.sourceNeighborhood.toScheme.toSpecΓ :=
    IsAffine.affine
  Over.isoMk (asIso C.sourceNeighborhood.toScheme.toSpecΓ)
    C.neighborhoodToSpecΓ_over

/-- Complex points of the affine source neighborhood as complex algebra homomorphisms on its
coordinate ring. -/
def neighborhoodPointAlgHomHomeomorph :
    @Homeomorph
      (ComplexPoint C.neighborhoodScheme)
      (Γ(C.sourceNeighborhood.toScheme, ⊤) →ₐ[ℂ] ℂ)
      Point.analyticTopology
      (ComplexPoint.affineAlgebraHomTopology Γ(C.sourceNeighborhood.toScheme, ⊤)) :=
  (Point.isoMapHomeomorph C.neighborhoodToSpecΓIso).trans
    (ComplexPoint.affineSpecHomeomorph Γ(C.sourceNeighborhood.toScheme, ⊤))
...
lemma neighborhoodPointAlgHomHomeomorph_apply
    ...
    (r : Γ(C.sourceNeighborhood.toScheme, ⊤)) :
    C.neighborhoodPointAlgHomHomeomorph z r = Point.evaluate ⊤ r z := by
  change ComplexPoint.affineSpecEquiv Γ(C.sourceNeighborhood.toScheme, ⊤)
  ...
  change Point.evaluate ⊤
      (C.sourceNeighborhood.toScheme.toSpecΓ.appTop
        ((Scheme.ΓSpecIso (.of Γ(C.sourceNeighborhood.toScheme, ⊤))).inv r)) z = _
  rw [Scheme.toSpecΓ_appTop]
  have h := DFunLike.congr_fun (congrArg CommRingCat.Hom.hom
    (Scheme.ΓSpecIso (.of Γ(C.sourceNeighborhood.toScheme, ⊤))).inv_hom_id) r
...
lemma neighborhoodProjectionChart_apply_of_mem
    ...
    C.neighborhoodProjectionChart z =
      ComplexAlgHom.mvPolynomialAlgHomHomeomorph n
        (ComplexAlgHom.etaleBaseAlgHom Γ(C.sourceNeighborhood.toScheme, ⊤)
...
/-- Evaluation of a global section of the affine source neighborhood is analytic along the
...
lemma analyticAt_neighborhoodProjectionChart_symm_evaluate_top
  ...
  have hw' : w ∈ (ComplexAlgHom.etaleAlgHomProjectionChart
      Γ(C.sourceNeighborhood.toScheme, ⊤) u).target := by
  ...
  have h := ComplexAlgHom.analyticAt_etaleAlgHomProjectionChart_symm_apply
    Γ(C.sourceNeighborhood.toScheme, ⊤) u hw' r
...
lemma analyticAt_neighborhoodProjectionChart_symm_evaluate
    {w : Fin n → ℂ} (hw : w ∈ C.neighborhoodProjectionChart.target)
    (W : C.sourceNeighborhood.toScheme.Opens) (s : Γ(C.sourceNeighborhood.toScheme, W))
    (hW : C.neighborhoodProjectionChart.symm w ∈ Point.overOpen W) :
    AnalyticAt ℂ (fun v ↦ Point.evaluate W s
      (C.neighborhoodProjectionChart.symm v)) w := by
  let Y := C.neighborhoodScheme
  let : IsAffine Y.left := C.sourceNeighborhood_isAffine
