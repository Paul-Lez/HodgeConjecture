...
through the relative/supported-singular comparison and the literal
...
local instance : ∀ V : Opens (ComplexPoint X), ParacompactSpace V := openParacompactSpace X
...
theorem complexSupportSingularCohomologySheafIsoRelative_restriction_section
    ...
    HomologicalComplex.homologyMap
      (TopCat.Sheaf.sectionComplexRestriction (TopCat.of (ComplexPoint X)) ℤᵘᵖ
        (complexSupportSingularComplex X S) a) (n : ℤ) ≫
      TopCat.Sheaf.sectionCohomologyToSheafSection (TopCat.of (ComplexPoint X))
        (complexSupportSingularComplex X S) (n : ℤ) W ≫
      (supportedSingularCohomologySheafIsoRelative (TopCat.of (ComplexPoint X)) S S.isClosed n).hom.hom.app (op W) =
...
theorem cycleComponentSupportedClassNormalizationIso_hom :
    (cycleComponentSupportedClassNormalizationIso X x hx).hom =
      ...
      (supportedSingularCohomologySheafIsoRelative (TopCat.of (ComplexPoint X))
        ...
        (cycleComponentAnalyticClosedSupport X x).isClosed (2 * p)).hom.hom.app
...
/-- The old point coclass transported through the ACTUAL relative/supported-singular
comparison; this is an explicit comparison target, not the definition of the general class. -/
def analyticComponentPointSupportedSingularCoclass :
  ...
  (complexSupportSingularSectionCohomologyEquiv X
  ...
  (rationalCohomologyAddEquivAmbientInjectiveHomology X (2 * d)).symm
    (HomologicalComplex.homologyMap
      ...
      ((complexSupportedSingularInjectiveHomologyIso X
        (cycleComponentAnalyticClosedSupport X x).compl ⊤ (2 * (d : ℤ))).hom
          (analyticComponentPointSupportedSingularCoclass X x d z)))

/-- The comparison target retains exactly the old normalized relative point coclass. -/
@[simp]
theorem analyticComponentPointSupportedSingularCoclass_relative :
    complexSupportSingularSectionCohomologyEquiv X
...
theorem analyticComponentPointSupportedSingularCoclass_section_normalization
    (hx : Order.coheight x = d) :
    (supportedSingularCohomologySheafIsoRelative (TopCat.of (ComplexPoint X))
      (cycleComponentAnalyticClosedSupport X x)
      (cycleComponentAnalyticClosedSupport X x).isClosed (2 * d)).hom.hom.app
      ...
      (TopCat.Sheaf.sectionCohomologyToSheafSection (TopCat.of (ComplexPoint X))
        ...
        (HomologicalComplex.homologyMap (cycleComponentSupportSectionRestriction X x)
          (2 * (d : ℤ)) (analyticComponentPointSupportedSingularCoclass X x d z))) =
  ...
  have h := ConcreteCategory.congr_hom
    (complexSupportSingularCohomologySheafIsoRelative_restriction_section X
      (cycleComponentAnalyticClosedSupport X x) (2 * dim X.left)
      (homOfLE (show cycleComponentSmoothSupportAmbientOpen X x ≤ ⊤ from le_top)))
        (analyticComponentPointSupportedSingularCoclass X x (dim X.left) z)
  refine h.trans ?_
  change (supportRelativeCohomologySheaf (TopCat.of (ComplexPoint X))
    ...
    ((supportRelativeCohomologyToSheaf (TopCat.of (ComplexPoint X))
      ...
      (complexSupportSingularSectionCohomologyEquiv X
        (cycleComponentAnalyticClosedSupport X x) ⊤ (2 * dim X.left)
        (analyticComponentPointSupportedSingularCoclass X x (dim X.left) z))) = _
  rw [analyticComponentPointSupportedSingularCoclass_relative,
...
coclass transported through the actual relative/singular comparison. This is
a uniqueness theorem about the general construction, not a point branch. -/
theorem cycleComponentSupportedSingularClass_point_normalization
    (hx : Order.coheight x = d) :
    cycleComponentSupportedSingularClass X x hx =
      analyticComponentPointSupportedSingularCoclass X x d z := by
  obtain rfl : dim X.left = d := SmoothOfRelativeDimension.dim_eq X.hom d
  symm
  apply cycleComponentSupportedSingularClass_unique X x hx
  ...
  exact analyticComponentPointSupportedSingularCoclass_section_normalization X x _ z hx
...
theorem coneCycleComponentSheafClass_point_normalization
  ...
  rw [coneCycleComponentSheafClass_eq_injectiveModel,
    cycleComponentSupportedSingularClass_point_normalization X x d z hx]
  rfl
