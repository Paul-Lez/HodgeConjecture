...
`h.hodgeStructure`, and proves that the Hodge classes of the statement, `Hdg^p(X; ℚ)`, are then
...
def HasHodgeDecomposition (n : ℕ) : Prop :=
  ∃ H : HodgeStructure.Pure (H^n(X; ℚ)) n,
...
theorem hasHodgeDecomposition_iff_isInternal (n : ℕ) :
    HasHodgeDecomposition X n ↔
      DirectSum.IsInternal fun pq : ℕ × ℕ ↦ HodgeStructure.Pure.pieceOfFiltration
        (fun p : ℕ ↦ complexifiedFieldHodgeFiltration ℚ X p n) n pq.1 pq.2 :=
  ⟨fun ⟨H, hH⟩ ↦ H.isInternal_pieceOfFiltration hH,
    fun h ↦ ⟨HodgeStructure.Pure.ofFiltration _ n h, fun _ hp ↦
      HodgeStructure.Pure.filtration_ofFiltration h (complexifiedFieldHodgeFiltration_antitone ℚ X n) hp⟩⟩
...
theorem hasHodgeDecomposition_zero : HasHodgeDecomposition X 0 := by
  have hF : complexifiedFieldHodgeFiltration ℚ X 0 0 = ⊤ := by
    rw [complexifiedFieldHodgeFiltration,
      hodgeFiltrationComplexSubmodule_zero_eq_top, Submodule.comap_top]
  have key : (fun pq : ℕ × ℕ ↦ HodgeStructure.Pure.pieceOfFiltration
      (fun p : ℕ ↦ complexifiedFieldHodgeFiltration ℚ X p (0 : ℕ)) 0 pq.1 pq.2) =
      fun pq ↦ (HodgeStructure.Pure.weightZero (H^(0 : ℕ)(X; ℚ))).piece pq.1 pq.2 := by
    funext pq
    rw [HodgeStructure.Pure.piece_weightZero]
    split_ifs with hpq
    · obtain ⟨h₁, h₂⟩ : pq.1 = 0 ∧ pq.2 = 0 := by omega
      rw [HodgeStructure.Pure.pieceOfFiltration_of_add_eq hpq, h₁, h₂]
      simp [hF]
    · exact HodgeStructure.Pure.pieceOfFiltration_of_add_ne hpq
  rw [hasHodgeDecomposition_iff_isInternal]
  show DirectSum.IsInternal fun pq : ℕ × ℕ ↦ HodgeStructure.Pure.pieceOfFiltration
    (fun p : ℕ ↦ complexifiedFieldHodgeFiltration ℚ X p (0 : ℕ)) 0 pq.1 pq.2
  rw [key]
  exact (HodgeStructure.Pure.weightZero _).isInternal
...
theorem hasHodgeDecomposition_iff_of_bijective {n : ℕ}
  ...
  have key : (fun pq : ℕ × ℕ ↦ HodgeStructure.Pure.pieceOfFiltration
    ...
    split_ifs with hpq
    · rw [HodgeStructure.Pure.pieceOfFiltration_of_add_eq hpq, ← complexifiedFieldHodgePiece_eq_inf_comap]
      rfl
    · rw [HodgeStructure.Pure.pieceOfFiltration_of_add_ne hpq, Submodule.comap_bot]
      exact e.ker.symm
  rw [hasHodgeDecomposition_iff_isInternal, key,
    DirectSum.isInternal_submodule_iff_iSupIndep_and_iSup_eq_top,
    DirectSum.isInternal_submodule_iff_iSupIndep_and_iSup_eq_top]
  apply and_congr (iSupIndep_map_orderIso_iff (Submodule.orderIsoMapComap e).symm)
  rw [Function.comp_def, ← OrderIso.map_iSup, map_eq_top_iff]
...
def hodgeStructure : HodgeStructure.Pure (H^n(X; ℚ)) n :=
  HodgeStructure.Pure.ofFiltration _ n ((hasHodgeDecomposition_iff_isInternal X n).1 h)

lemma hodgeStructure_piece {p q : ℕ} (hpq : p + q = n) :
    h.hodgeStructure.piece p q = complexifiedFieldHodgePiece ℚ X p q n := by
  change HodgeStructure.Pure.pieceOfFiltration
...
lemma hodgeStructure_filtration {p : ℕ} (hp : p ≤ n) :
  ...
  HodgeStructure.Pure.filtration_ofFiltration _ (complexifiedFieldHodgeFiltration_antitone ℚ X n) hp

/-- A pure Hodge structure on `H^n(X; ℚ)` with the de Rham Hodge filtration is the one built from
the Hodge decomposition. -/
theorem eq_hodgeStructure (H : HodgeStructure.Pure (H^n(X; ℚ)) n)
...
`HodgeStructure.Pure.filtration_inf_conjugateFiltration` identifies `F^p ⊓ conj F^p` with the `(p,p)`
piece. -/
theorem hodgeClasses_eq_hodgeClasses_of_filtration_eq (p : ℕ)
    (H : HodgeStructure.Pure (H^(2 * p)(X; ℚ)) (2 * p))
    (hH : H.filtration p = complexifiedFieldHodgeFiltration ℚ X p (2 * p)) :
    Hdg^p(X; ℚ) = H.hodgeClasses p := by
  rw [hodgeClasses_eq_comap_ofBase, HodgeStructure.Pure.hodgeClasses,
    ← H.filtration_inf_conjugateFiltration p, H.conjugateFiltration_eq_comap, hH,
...
theorem hodgeClasses_eq_hodgeClasses_hodgeStructure (p : ℕ)
    ...
    Hdg^p(X; ℚ) = h.hodgeStructure.hodgeClasses p :=
...
theorem hodgeClasses_zero_eq_hodgeClasses_hodgeStructure :
    Hdg^0(X; ℚ) = (hasHodgeDecomposition_zero X).hodgeStructure.hodgeClasses 0 :=
