...
The Hodge filtration is decreasing. The comparison map `H^n(X; K) → H^n_dR(X)` extends to a complex-linear map
...
None of this is needed to state the conjecture. `Other.AlgebraicGeometry.Hodge.Decomposition` uses
...
def fieldToDeRhamComplexificationBilinear (n : ℕ) :
...
def fieldToDeRhamComplexification (n : ℕ) :
...
@[simp] lemma fieldToDeRhamComplexification_tmul (n : ℕ) (c : ℂ) (α : H^n(X; K)) :
...
def complexifiedFieldHodgePiece (p q : ℤ) (n : ℕ) : Submodule ℂ (ℂ ⊗[K] H^n(X; K)) :=
...
decreasing. -/
lemma complexifiedFieldHodgeFiltration_antitone (n : ℕ) :
...
lemma fieldToDeRhamComplexification_conjugate (n : ℕ) (x : ℂ ⊗[ℚ] H^n(X; ℚ)) :
...
lemma complexifiedFieldHodgePiece_eq_inf_comap (p q : ℤ) (n : ℕ) :
    complexifiedFieldHodgePiece ℚ X p q n =
      complexifiedFieldHodgeFiltration ℚ X p n ⊓
        (complexifiedFieldHodgeFiltration ℚ X q n).comap (HodgeStructure.conjugateSemilinear _) := by
  ext x
  change (_ ∈ hodgeFiltration X p n ∧
      deRhamConj X n (fieldToDeRhamComplexification ℚ X n x) ∈ hodgeFiltration X q n) ↔
    (_ ∈ hodgeFiltration X p n ∧
      fieldToDeRhamComplexification ℚ X n (HodgeStructure.conjugate _ x) ∈
        hodgeFiltration X q n)
  rw [fieldToDeRhamComplexification_conjugate]
...
lemma hodgeClasses_eq_comap_ofBase (p : ℕ) :
  ...
  ext α
  change fieldToDeRhamCohomology K X (2 * p) α ∈ hodgePiece X p p (2 * p : ℕ) ↔
    fieldToDeRhamComplexification K X (2 * p)
      (HodgeStructure.ofBase K (H^(2 * p)(X; K)) α) ∈ hodgePiece X p p (2 * p : ℕ)
