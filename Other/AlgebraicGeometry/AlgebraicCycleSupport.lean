...
The geometric support of an algebraic cycle is the union of the closures of the generic points
carrying a nonzero coefficient, and the analytic support is the set of complex points lying over
it. Both are closed, and the analytic support of the pushforward of a principal divisor lies over
the carrier of that divisor.

The statement of the Hodge conjecture only needs the support of a single irreducible component,
which is `HodgeConjecture.Definitions.AlgebraicGeometry.AlgebraicCycleSupport`; a whole cycle
enters only through the Chow-group material, which lives here.
-/
