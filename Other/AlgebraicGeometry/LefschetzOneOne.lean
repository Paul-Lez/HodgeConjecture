...
@[expose] public def RationalLefschetzOneOne : Prop :=
  ∀ {X : Scheme} [IsIntegral X] (structureMap : X ⟶ Spec ↧ℂ) [Smooth structureMap]
    [IsProjective structureMap] (α : FieldCohomology ℚ structureMap 2),
    α ∈ Hdg^1(ℚ; structureMap) →
      ∃ D : TensorProduct ℤ ℚ (CodimensionCycle X 1),
        rationalSheafCycleClassOnCycles
          (DimensionedSmoothProjectiveComplexVariety.ofStructureMap structureMap) 1 D = α
...
public theorem HodgeConjecture.rationalLefschetzOneOne
  ...
  intro X _ structureMap _ _ α hα
  exact algebraicCycleClassSpan_le_range_rationalSheafCycleClassOnCycles structureMap 1
    (hodge structureMap 1 hα)
...
public theorem HodgeConjecture.rationalLefschetzOneOne_direct :
  ...
  intro hodge X _ structureMap _ _ α hα
  have hspan : algebraicCycleClassSpan structureMap 1 ≤
      LinearMap.range (rationalSheafCycleClassOnCycles
        (DimensionedSmoothProjectiveComplexVariety.ofStructureMap structureMap) 1) := by
    ...
    refine (rationalSheafCycleClassOnCycles_tmul_single
      (DimensionedSmoothProjectiveComplexVariety.ofStructureMap structureMap) 1 1 x hx).trans ?_
    rw [one_smul]
    rfl
  exact hspan (hodge structureMap 1 hα)
