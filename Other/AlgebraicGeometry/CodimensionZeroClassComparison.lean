...
component class of the generic point, and the genuine Chow-group span is the line generated by
the degree-zero cohomological unit. On a connected analytification degree-zero rational
cohomology is one dimensional, so the two lines agree exactly when the generic-point component
class is nonzero.
...
cohomology, given connectedness and nonvanishing of the normalized coclass section. -/
...
/-- The degree-zero Hodge classes are exactly the algebraic cycle-class span, given connectedness
and nonvanishing of the normalized coclass section. -/
...
/-- The Hodge-conjecture inclusion in codimension zero, given connectedness and nonvanishing of
...
theorem rationalHodgeClasses_zero_le_algebraicCycleClassSpan_of_coclassSection_ne_zero
    ...
    (hne : cycleComponentSmoothSupportCoclassSection X (genericPoint X.left)
      (d := dim X.left) (coheight_genericPoint_eq_zero X) ≠ 0) :
    Hdg^0(ℚ; X) ≤ algebraicCycleClassSpan X 0 := by
  rw [rationalHodgeClasses_zero_eq_algebraicCycleClassSpan_of_coclassSection_ne_zero X hV hne]
