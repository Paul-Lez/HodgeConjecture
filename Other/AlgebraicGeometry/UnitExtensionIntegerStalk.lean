...
def integerOneRestrict (U : Opens (TopCat.of (ComplexPoint X))) :
    (𝓒(↧(ComplexPoint X); ℤ)).obj.obj (op U) :=
  (𝓒(↧(ComplexPoint X); ℤ)).obj.map (homOfLE (le_top : U ≤ ⊤)).op (integerOneSection (X := X))

theorem integerOneRestrict_map {U V : Opens (TopCat.of (ComplexPoint X))} (h : V ≤ U) :
    (𝓒(↧(ComplexPoint X); ℤ)).obj.map (homOfLE h).op (integerOneRestrict X U) =
...
def integerStalkIso (x : ComplexPoint X) :
    AddCommGrpCat.of ℤ ≅ (𝓒(↧(ComplexPoint X); ℤ)).presheaf.stalk x :=
...
theorem integerStalkIso_hom_one (x : ComplexPoint X) :
    (integerStalkIso X x).hom (1 : ℤ) =
      (𝓒(↧(ComplexPoint X); ℤ)).presheaf.germ ⊤ x True.intro (integerOneSection (X := X)) := by
...
theorem integerStalkIso_hom_apply (x : ComplexPoint X) (n : ℤ) :
    (integerStalkIso X x).hom n =
      (𝓒(↧(ComplexPoint X); ℤ)).presheaf.germ ⊤ x True.intro
...
theorem integerOneSection_zsmul_germ_injective (x : ComplexPoint X) :
    Function.Injective (fun n : ℤ => (𝓒(↧(ComplexPoint X); ℤ)).presheaf.germ ⊤ x True.intro
      (n • integerOneSection (X := X))) := by
  intro a b hab
  simp only at hab
...
theorem exists_zsmul_integerOneSection (V : Opens (TopCat.of (ComplexPoint X)))
    (t : (𝓒(↧(ComplexPoint X); ℤ)).obj.obj (op V)) (x : ComplexPoint X) (hx : x ∈ V) :
    ∃ (U : Opens (TopCat.of (ComplexPoint X))) (_ : x ∈ U) (hUV : U ≤ V) (n : ℤ),
      (𝓒(↧(ComplexPoint X); ℤ)).obj.map (homOfLE hUV).op t = n • integerOneRestrict X U := by
  classical
  set n : ℤ := (integerStalkIso X x).inv
    ((𝓒(↧(ComplexPoint X); ℤ)).presheaf.germ V x hx t) with hn
  have hgerm : (𝓒(↧(ComplexPoint X); ℤ)).presheaf.germ V x hx t =
      (𝓒(↧(ComplexPoint X); ℤ)).presheaf.germ ⊤ x True.intro
        (n • integerOneSection (X := X)) := by
    rw [← integerStalkIso_hom_apply, hn]
    exact (CategoryTheory.Iso.inv_hom_id_apply (integerStalkIso X x) _).symm
  obtain ⟨W, hxW, iU, iV, hW⟩ :=
    (𝓒(↧(ComplexPoint X); ℤ)).presheaf.germ_eq (U := V) (V := ⊤) x hx True.intro t
      (n • integerOneSection (X := X)) hgerm
  refine ⟨W, hxW, iU.le, n, ?_⟩
  have hiU : (homOfLE iU.le : W ⟶ V) = iU := rfl
  have hgoal : (𝓒(↧(ComplexPoint X); ℤ)).obj.map iU.op t =
      (𝓒(↧(ComplexPoint X); ℤ)).obj.map iV.op (n • integerOneSection (X := X)) := hW
