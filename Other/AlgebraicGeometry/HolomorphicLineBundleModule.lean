...
local instance holomorphicLineBundleModuleTopology : TopologicalSpace (ComplexPoint X s) :=
  Point.analyticTopology

/-- The holomorphic structure sheaf, regarded as a sheaf of rings. -/
def holomorphicRingSheaf :
    Sheaf (Opens.grothendieckTopology (TopCat.of (ComplexPoint X s))) RingCat :=
  (sheafCompose (Opens.grothendieckTopology (TopCat.of (ComplexPoint X s)))
    (forget₂ CommRingCat RingCat)).obj (holomorphicFunctionSheaf s d)

namespace HolomorphicUnitExtension

variable {s d} (E : HolomorphicUnitExtension s d)

local instance sectionCoordinatesModule (U : Opens (TopCat.of (ComplexPoint X s))) :
...
theorem contMDiffAt_extendSection_scalar (U : Opens (TopCat.of (ComplexPoint X s)))
...
def sectionSubmodule (U : Opens (TopCat.of (ComplexPoint X s))) :
...
instance holomorphicSections_module (U : Opens (TopCat.of (ComplexPoint X s))) :
...
instance holomorphicSections_ringModule (U : (Opens (TopCat.of (ComplexPoint X s)))ᵒᵖ) :
    Module ((holomorphicRingSheaf s d).obj.obj U) (E.HolomorphicSections (unop U)) :=
    inferInstanceAs (Module (C^ω⟮𝓘(ℂ, Fin d → ℂ),
      (unop U : Opens (TopCat.of (ComplexPoint X s))); ℂ⟯) (E.HolomorphicSections (unop U)))

/-- The module of holomorphic sections with the structure sheaf's ring as scalar ring. -/
def sectionModule (U : (Opens (TopCat.of (ComplexPoint X s)))ᵒᵖ) :
    ModuleCat ((holomorphicRingSheaf s d).obj.obj U) :=
  ModuleCat.of _ (E.HolomorphicSections (unop U))

/-- Restriction of sections is linear after restricting scalars. -/
def sectionModuleRestriction {U V : (Opens (TopCat.of (ComplexPoint X s)))ᵒᵖ} (i : U ⟶ V) :
    E.sectionModule U ⟶ (ModuleCat.restrictScalars ((holomorphicRingSheaf s d).obj.map i).hom).obj
      (E.sectionModule V) := by
  letI : Module ((holomorphicRingSheaf s d).obj.obj U) (E.sectionModule V) :=
    Module.compHom (E.sectionModule V) ((holomorphicRingSheaf s d).obj.map i).hom
...
def sectionPresheafOfModules : PresheafOfModules (holomorphicRingSheaf s d).obj where
  obj := E.sectionModule
...
def sectionSheafOfModules : SheafOfModules (holomorphicRingSheaf s d) where
