...
/-!
# SupportSingularGlobal, the part the statement does not need

Separated out of
`HodgeConjecture.Lemmas.AlgebraicGeometry.Cohomology.SupportSingularGlobal`:
...
lemma globalRawSingularRestrictionInt_transport_relative (A : Set X) :
  ...
  exact HomologicalComplex.mapExtendCanonicalIso_inv_naturality
    (forget₂ (ModuleCat R) AddCommGrpCat)
    (HomologicalComplex.linearDualMap
      ((chainPairFunctor R).obj (TopPair.ofSubset A)).hom)
    ComplexShape.embeddingUpNat
