...
local instance holomorphicExponentialSequenceTopology : TopologicalSpace (ComplexPoint X s) :=
  Point.analyticTopology

/-- Integer constants as sections of the holomorphic-function presheaf. -/
def integerConstantsToHolomorphicPresheaf :
    (Functor.const (Opens (TopCat.of (ComplexPoint X s)))ᵒᵖ).obj (AddCommGrpCat.of ℤ) ⟶
      (holomorphicAdditiveSheaf s d).obj where
...
def integerConstantsToHolomorphicSheaf :
    constantIntegerSheaf s ⟶ holomorphicAdditiveSheaf s d :=
  ⟨sheafifyLift (Opens.grothendieckTopology (TopCat.of (ComplexPoint X s)))
    (integerConstantsToHolomorphicPresheaf s d) (holomorphicAdditiveSheaf s d).property⟩

@[reassoc (attr := simp)]
lemma toSheafify_integerConstantsToHolomorphicSheaf :
    toSheafify (Opens.grothendieckTopology (TopCat.of (ComplexPoint X s)))
        ((Functor.const (Opens (TopCat.of (ComplexPoint X s)))ᵒᵖ).obj (AddCommGrpCat.of ℤ)) ≫
      (integerConstantsToHolomorphicSheaf s d).hom =
        integerConstantsToHolomorphicPresheaf s d :=
  toSheafify_sheafifyLift ..

lemma integerConstantsToHolomorphicPresheaf_comp_exponential :
    integerConstantsToHolomorphicPresheaf s d ≫ (holomorphicExponential s d).hom = 0 := by
  apply NatTrans.ext
  funext U
  apply AddCommGrpCat.hom_ext
  apply AddMonoidHom.ext
...
lemma integerConstantsToHolomorphicSheaf_comp_exponential :
    integerConstantsToHolomorphicSheaf s d ≫ holomorphicExponential s d = 0 := by
  apply Sheaf.hom_ext
  apply sheafify_hom_ext _ _ _ (holomorphicUnitSheaf s d).property
  change _ ≫ (integerConstantsToHolomorphicSheaf s d).hom ≫
    (holomorphicExponential s d).hom = _ ≫ 0
...
def holomorphicExponentialPresheafSequence :
    ShortComplex (TopCat.Presheaf AddCommGrpCat (TopCat.of (ComplexPoint X s))) :=
  ShortComplex.mk (integerConstantsToHolomorphicPresheaf s d)
    (holomorphicExponential s d).hom
    (integerConstantsToHolomorphicPresheaf_comp_exponential s d)

/-- The exponential sequence of analytic sheaves. -/
def holomorphicExponentialSequence :
    ShortComplex (TopCat.Sheaf AddCommGrpCat (TopCat.of (ComplexPoint X s))) :=
  ShortComplex.mk (integerConstantsToHolomorphicSheaf s d) (holomorphicExponential s d)
    (integerConstantsToHolomorphicSheaf_comp_exponential s d)

set_option backward.isDefEq.respectTransparency false in
/-- Sheafifying the integer term leaves the other two terms unchanged. -/
def holomorphicExponentialSequenceSheafificationUnit :
    holomorphicExponentialPresheafSequence s d ⟶
      (holomorphicExponentialSequence s d).map
        (TopCat.Sheaf.forget AddCommGrpCat (TopCat.of (ComplexPoint X s))) where
  τ₁ := toSheafify (Opens.grothendieckTopology (TopCat.of (ComplexPoint X s))) _
  τ₂ := 𝟙 _
  τ₃ := 𝟙 _
  comm₁₂ := by
    change _ ≫ (integerConstantsToHolomorphicSheaf s d).hom =
      integerConstantsToHolomorphicPresheaf s d ≫ 𝟙 _
    rw [comp_id]
    exact toSheafify_integerConstantsToHolomorphicSheaf s d
  comm₂₃ := by
    rw [id_comp, comp_id]
    rfl
...
theorem holomorphicExponentialPresheafSequence_stalk_exact (x : ComplexPoint X s) :
    ((holomorphicExponentialPresheafSequence s d).map
...
theorem holomorphicExponentialSequence_exact : (holomorphicExponentialSequence s d).Exact := by
  ...
  let η := (stalk.mapShortComplex).map (holomorphicExponentialSequenceSheafificationUnit s d)
  have : IsIso η.τ₁ :=
  ...
  exact ShortComplex.exact_of_iso (asIso η)
    (holomorphicExponentialPresheafSequence_stalk_exact s d x)

set_option backward.isDefEq.respectTransparency false in
set_option backward.isDefEq.respectTransparency.types false in
/-- Integer constants remain distinct as germs of holomorphic functions. -/
theorem integerConstantsToHolomorphicPresheaf_stalk_mono (x : ComplexPoint X s) :
    Mono ((TopCat.Presheaf.stalkFunctor AddCommGrpCat x).map
      (integerConstantsToHolomorphicPresheaf s d)) := by
  rw [AddCommGrpCat.mono_iff_injective]
  let P : TopCat.Presheaf AddCommGrpCat (TopCat.of (ComplexPoint X s)) :=
    (Functor.const (Opens (TopCat.of (ComplexPoint X s)))ᵒᵖ).obj (AddCommGrpCat.of ℤ)
  intro z z' h
  ...
  obtain ⟨W, hxW, iWU, iWV, hW⟩ :=
    (holomorphicAdditiveSheaf s d).presheaf.germ_eq x hxU hxV _ _ h
  change ℤ at n n'
  have hn : (n : ℤ) = n' := by
    have hc := congrArg
...
instance integerConstantsToHolomorphicSheaf_mono :
    Mono (integerConstantsToHolomorphicSheaf s d) := by
  rw [TopCat.Presheaf.mono_iff_stalk_mono]
  intro x
  let stalk := TopCat.Presheaf.stalkFunctor AddCommGrpCat x
  let S := (holomorphicExponentialPresheafSequence s d).map stalk
  let T := (holomorphicExponentialSequence s d).map
    (TopCat.Sheaf.forget AddCommGrpCat (TopCat.of (ComplexPoint X s)) ⋙ stalk)
  let η : S ⟶ T :=
    (stalk.mapShortComplex).map (holomorphicExponentialSequenceSheafificationUnit s d)
  have : IsIso η.τ₁ :=
    TopCat.Presheaf.stalkFunctor_map_unit_toSheafify_isIso x AddCommGrpCat _
  have : IsIso η.τ₂ := by
    change IsIso (stalk.map (𝟙 _))
    rw [stalk.map_id]
    infer_instance
  have : Mono S.f := integerConstantsToHolomorphicPresheaf_stalk_mono s d x
  change Mono T.f
...
theorem holomorphicExponentialSequence_shortExact :
  ...
  mono_f := integerConstantsToHolomorphicSheaf_mono s d
  epi_g := holomorphicExponential_epi s d
