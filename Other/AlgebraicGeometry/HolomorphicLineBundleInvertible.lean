...
local instance holomorphicLineBundleInvertibleTopology : TopologicalSpace (ComplexPoint X) :=
  Point.analyticTopology

variable (E : HolomorphicUnitExtension X d) (i : ComplexPoint X)
...
def localSectionModuleIso (V : (Over (E.localLifts.opens i))ᵒᵖ) :
    ((E.sectionSheafOfModules).over (E.localLifts.opens i)).val.obj V ≅
      (SheafOfModules.unit ((holomorphicRingSheaf X d).over (E.localLifts.opens i))).val.obj V :=
...
def localSectionSheafIso : (E.sectionSheafOfModules).over (E.localLifts.opens i) ≅
    SheafOfModules.unit ((holomorphicRingSheaf X d).over (E.localLifts.opens i)) :=
...
def localFreeSheafIso :
    SheafOfModules.free (R := (holomorphicRingSheaf X d).over (E.localLifts.opens i)) PUnit ≅
...
theorem localLifts_coversTop :
    (Opens.grothendieckTopology (TopCat.of (ComplexPoint X))).CoversTop E.localLifts.opens := by
  intro U x hx
