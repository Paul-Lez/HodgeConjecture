...
`4484527579f75da6f50daa78a99f813aa4d38e6d` and
...
public abbrev FirstQuadrantBicomplex (C : Type 1 := AddCommGrpCat)
    [Category C] [Preadditive C] :=
  HomologicalComplex₂ C (ComplexShape.down ℕ) (ComplexShape.down ℕ)

public abbrev FirstQuadrantChainComplex (C : Type 1 := AddCommGrpCat)
    [Category C] [Preadditive C] :=
  HomologicalComplex C (ComplexShape.down ℕ)
...
public def firstQuadrantTotalFiberDiagramFunctor (n : ℕ) :
    FirstQuadrantBicomplex ⥤ (Discrete (FirstQuadrantTotalFiber n) ⥤ C) where
...
public def firstQuadrantTotalFiberEvaluationIso (n : ℕ)
    ...
    firstQuadrantTotalFiberDiagramFunctor (C := C) n ⋙
        (evaluation (Discrete (FirstQuadrantTotalFiber n)) C).obj pq ≅
      HomologicalComplex.eval FirstQuadrantChainComplex (ComplexShape.down ℕ) pq.as.1.1 ⋙
        HomologicalComplex.eval C (ComplexShape.down ℕ) pq.as.1.2 :=
  NatIso.ofComponents (fun _ => Iso.refl _) (by
    intro X Y f
    change (f.f pq.as.1.1).f pq.as.1.2 ≫ 𝟙 _ =
      𝟙 _ ≫ (f.f pq.as.1.1).f pq.as.1.2
    simp)

noncomputable instance (n : ℕ) :
    PreservesFiniteLimits (firstQuadrantTotalFiberDiagramFunctor (C := C) n) :=
  preservesFiniteLimits_of_evaluation _ fun pq =>
    preservesFiniteLimits_of_natIso
      (firstQuadrantTotalFiberEvaluationIso (C := C) n pq).symm

noncomputable instance (n : ℕ) :
    PreservesFiniteColimits (firstQuadrantTotalFiberDiagramFunctor (C := C) n) :=
...
public noncomputable def firstQuadrantTotalDegreeFunctor (n : ℕ) :
    FirstQuadrantBicomplex ⥤ C :=
  firstQuadrantTotalFiberDiagramFunctor (C := C) n ⋙
    colim (J := Discrete (FirstQuadrantTotalFiber n)) (C := C)

noncomputable instance (n : ℕ) :
    PreservesFiniteLimits (firstQuadrantTotalDegreeFunctor (C := C) n) := by
  dsimp only [firstQuadrantTotalDegreeFunctor]
  infer_instance

noncomputable instance (n : ℕ) :
    PreservesFiniteColimits (firstQuadrantTotalDegreeFunctor (C := C) n) := by
  dsimp only [firstQuadrantTotalDegreeFunctor]
  infer_instance
...
public noncomputable def firstQuadrantTotalEvaluationIso (n : ℕ) :
    HomologicalComplex₂.totalFunctor C
        (ComplexShape.down ℕ) (ComplexShape.down ℕ) (ComplexShape.down ℕ) ⋙
      HomologicalComplex.eval C (ComplexShape.down ℕ) n ≅
    firstQuadrantTotalDegreeFunctor (C := C) n :=
  NatIso.ofComponents (fun _ => Iso.refl _) (by
    intro X Y f
    change (HomologicalComplex₂.total.map f (ComplexShape.down ℕ)).f n ≫ 𝟙 _ =
      𝟙 _ ≫ (HomologicalComplex₂.total.map f (ComplexShape.down ℕ)).f n
    simp)
...
noncomputable instance : PreservesFiniteLimits
  ...
  refine ⟨by
    ...
    exact preservesLimitsOfShape_of_natIso
      (J := J) (firstQuadrantTotalEvaluationIso (C := C) n).symm⟩
...
public theorem firstQuadrantTotal_shortExact
    ...
    (S.map (HomologicalComplex₂.totalFunctor C
