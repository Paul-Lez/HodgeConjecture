...
public import Other.AlgebraicTopology.NormalChartWindingBoundary
public import HodgeConjecture.Lemmas.AlgebraicTopology.NormalProjectionCoclass
...
theorem complexLineCoordinate_standardRealToComplex (v : (standardPuncturedPair 2).snd) :
  ...
  change ((Complex.piCoordCLE 1).symm (Subtype.val v)) 0 =
    complexCoordinateFun (Subtype.val v)
  have b0 : (Complex.piBasisOneI 1) 0 0 = 1 := by
    have h : finProdFinEquiv.symm (0 : Fin 2) = ((0 : Fin 1), (0 : Fin 2)) := by decide
    simp [Complex.piBasisOneI, Module.Basis.reindex_apply,
      Module.Basis.smulTower'_apply, Complex.basisOneI, Module.Basis.ofEquivFun, h]
  have b1 : (Complex.piBasisOneI 1) 1 0 = Complex.I := by
    have h : finProdFinEquiv.symm (1 : Fin 2) = ((0 : Fin 1), (1 : Fin 2)) := by decide
    simp [Complex.piBasisOneI, Module.Basis.reindex_apply,
      Module.Basis.smulTower'_apply, Complex.basisOneI, Module.Basis.ofEquivFun, h]
  rw [show (Complex.piCoordCLE 1).symm (Subtype.val v) =
      ∑ i, (Subtype.val v) i • Complex.piBasisOneI 1 i by
      exact Module.Basis.equivFun_symm_apply _ _]
  rw [Fin.sum_univ_two]
  simp only [Pi.add_apply, Pi.smul_apply]
  rw [b0, b1]
  simp [complexCoordinateFun]
