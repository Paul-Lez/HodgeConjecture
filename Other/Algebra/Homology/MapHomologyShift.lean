...
theorem map_shiftShortComplex_middle (K : CochainComplex C ℤ)
    (t n n' : ℤ) (h : t + n = n') :
    (((HomologicalComplex.shortComplexFunctor D ℤᵘᵖ n).map
      (((F.mapHomologicalComplex ℤᵘᵖ).commShiftIso t).hom.app K)) ≫
        (shiftShortComplexFunctorIso D t n n' h).hom.app
          ((F.mapHomologicalComplex ℤᵘᵖ).obj K)).τ₂ =
...
theorem mapHomologyIso_shift (K : CochainComplex C ℤ)
    ...
    HomologicalComplex.homologyMap
        (((F.mapHomologicalComplex ℤᵘᵖ).commShiftIso t).hom.app K) n ≫
      ((HomologicalComplex.homologyFunctor D ℤᵘᵖ 0).shiftIso t n n' h).hom.app
        ((F.mapHomologicalComplex ℤᵘᵖ).obj K) ≫
      ((K.sc n').mapHomologyIso F).hom =
    (((K⟦t⟧).sc n).mapHomologyIso F).hom ≫
      F.map (((HomologicalComplex.homologyFunctor C ℤᵘᵖ 0).shiftIso t n n' h).hom.app K) := by
  ...
  change HomologicalComplex.homologyMap
    ...
    ShortComplex.homologyMap ((shiftShortComplexFunctorIso D t n n' h).hom.app
      ((F.mapHomologicalComplex ℤᵘᵖ).obj K)) = _ at hh
...
theorem mapHomologyIso_shiftMap {K L : CochainComplex C ℤ}
    ...
    (HomologicalComplex.homologyFunctor D ℤᵘᵖ 0).shiftMap
        (ShiftedHom.map f (F.mapHomologicalComplex ℤᵘᵖ)) n n' h ≫
      ((L.sc n').mapHomologyIso F).hom =
    ((K.sc n).mapHomologyIso F).hom ≫
      F.map ((HomologicalComplex.homologyFunctor C ℤᵘᵖ 0).shiftMap f n n' h) := by
  ...
  have hn := ShortComplex.mapHomologyIso_hom_naturality
    ((HomologicalComplex.shortComplexFunctor C ℤᵘᵖ n).map f) F
  change (HomologicalComplex.homologyFunctor D ℤᵘᵖ n).map
      ((F.mapHomologicalComplex ℤᵘᵖ).map f) ≫
    (((L⟦t⟧).sc n).mapHomologyIso F).hom =
      ((K.sc n).mapHomologyIso F).hom ≫
        F.map ((HomologicalComplex.homologyFunctor C ℤᵘᵖ n).map f) at hn
  erw [hn]
