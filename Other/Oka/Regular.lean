...
(`LocalOkaRing.isLocalWeierstrassPolynomial_X`), so `Other/Oka/Weierstrass.lean`'s division theorem
...
(`Other/Oka/StalkEquiv.lean`, root namespace), and the bridge
...
**Nothing under `Other/Oka/` produces a partial derivative *as a germ*, and that is the same fact as the
...
`Other/Oka/AnalyticSpace/SimpleZeroPolynomial.lean` and the implicit-function result of
`Other/Oka/Weierstrass.lean` among them.

**The tally that used to stand here is deliberately gone.** This paragraph has carried a
tree-wide count twice ; *"the one declaration under `Other/Oka/` that computes a partial derivative"*,
...
g * u`; `partialEval_coe_fromPolynomial` (`Other/Oka/Weierstrass.lean`, root namespace) and
`LocalOkaRing.map_constantCoeff_eq_X_pow` (`Other/Oka/UFD.lean`) turn the first factor into
