...
The `ℂ`-linearity API, which is used throughout `Other/Oka/AnalyticSpace/` and lived in three
...
- `ComplexAnalytic.isCLinearHom_ofRestrict`: **the inclusion of an open subspace is `ℂ`-linear**
  ...
  in. `Other/Oka/AnalyticSpace/LocalModel.lean` restates it for an open subset of `ℂ^n` and the
...
- `ComplexAnalytic.AnalyticSpace.ext'`: **two analytic spaces with the same underlying locally
  ...
  **Its only use site at the commit that adds it is `ComplexAnalytic.restrict_eq_ofCutOut`** in
  `Other/Oka/AnalyticSpace/OpenSubspace.lean`, where the proof spells the name `AnalyticSpace.ext'` and
...
`ComplexAnalytic.IsCutOutBy.lift` in `Other/Oka/AnalyticSpace/Factorisation.lean` builds it from
...
`ComplexAnalytic.restrict_eq_ofCutOut` in `Other/Oka/AnalyticSpace/OpenSubspace.lean` is the consumer
...
`Other/Oka/Analytification/StandardEtaleLocalIso.lean` records the same functor as faithful and not
...
are different discrimination-tree keys, so the `haveI` in the proof below is essential: the
...
(`Other/Oka/AnalyticSpace/LocalIso.lean`) does. The same seam is recorded by the docstrings of
...
(`Other/Oka/AnalyticSpace/Sigma.lean`) already writes that caveat for its own statement, and every
...
`ComplexAnalytic.AnalyticSpace.degree_isIso_comp` (`Other/Oka/AnalyticSpace/Degree.lean`) is the
...
consumer; `ComplexAnalytic.AnalyticSpace.not_isIso_sigmaι` (`Other/Oka/AnalyticSpace/Sigma.lean`)
...
(`Other/Oka/AnalyticSpace/FiniteEtaleOver.lean`) separates covers by a number, and two covers of the
