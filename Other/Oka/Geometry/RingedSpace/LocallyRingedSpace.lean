...
`Other/Oka/Geometry/RingedSpace/OpenImmersion.lean` where `restrictLE` consumes it: nothing about it
...
`Other/Oka/Geometry/RingedSpace/PresheafedSpace/Gluing.lean`. Stated with the carrier being empty
...
`Other/Oka/AnalyticSpace/Restrict.lean` runs through. Its point is that it needs **no equation of
