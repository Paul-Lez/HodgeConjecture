...
`python3 scripts/import_cost.py Other/Oka/AlgebraicGeometry/Modules/Tilde.lean`, not estimated, and
...
`Other/Oka/Algebra/Category/ModuleCat/Sheaf/Coherent/Presentation.lean` is a mirror file for a Mathlib
...
be written, and they are in `Other/Oka/AlgebraicGeometry/Modules/Sheaf.lean`.
...
`Other/Oka/AlgebraicGeometry/Modules/Coherent.lean`, proves `𝒪_{Spec A}` coherent for noetherian `A`. The
...
* **It is not available at the same hypotheses.** `Module.FinitePresentation R (Γ M)` is **false**
  ...
  it is recorded, as `Other/Oka/AnalyticSpace/Finite.lean` records `Spec k ⟶ Spec K`, because it is the
...
`Other/Oka/Algebra/Category/ModuleCat/Sheaf/Quasicoherent.lean` for why it does anyway. -/
