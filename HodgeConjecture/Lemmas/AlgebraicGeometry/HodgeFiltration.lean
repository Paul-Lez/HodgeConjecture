...
`HodgeConjecture.Definitions.AlgebraicGeometry.HodgeFiltration`.
-/
...
lemma restrictScalars_hodgeFiltrationComplexSubmodule_zero_eq_top [IsIntegral X.left]
    [Smooth X.hom] (n : ℤ) :
    (hodgeFiltrationComplexSubmodule X 0 n).restrictScalars K = ⊤ := by
