...
Like `ComplexPoint.Localization`, this works throughout with `ℂ`-algebra homomorphisms rather
than with complex points of schemes, so it lives in namespace `AlgebraicGeometry.ComplexAlgHom`.
-/
...
def standardEtalePointToAlgHom (z : standardEtalePointSpace P) : P.Ring →ₐ[ℂ] ℂ :=
