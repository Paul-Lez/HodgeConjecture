...
def sheafSectionsBetweenOpensOnOpenIso (W : Opens X) (F : Sheaf AddCommGrpCat.{u} X) :
  ...
  PreservesKernel.iso Γ_[W] ((openRestrictionPushforwardMap X h).app F)
...
lemma sheafSectionsBetweenOpensOnOpenIso_hom_ι (W : Opens X)
    ...
    (sheafSectionsBetweenOpensOnOpenIso X h W F).hom ≫
      ...
      (kernel.ι ((openRestrictionPushforwardMap X h).app F)).hom.app (op W) :=
  kernelComparison_comp_ι ((openRestrictionPushforwardMap X h).app F) Γ_[W]

/-- Restrict an ambient section vanishing on `V` to its section on `U`. -/
def toSheafSectionsBetweenOpens :
    sectionsSupportedOutside X V ⟶ sheafSectionsBetweenOpens X h where
  ...
  naturality F G f := by
    ...
    simp only [sectionsSupportedOutside, sheafSectionsBetweenOpens, kernel.map,
...
@[reassoc (attr := simp)]
lemma sectionsSupportedOutsideMap_toBetween :
    sectionsSupportedOutsideMap X h ≫ toSheafSectionsBetweenOpens X h = 0 := by
  ext F : 2
  apply (cancel_mono (kernel.ι _)).1
  simp [toSheafSectionsBetweenOpens, sectionsSupportedOutsideMap,
    liftSectionsSupportedOutside, sectionsSupportedOutsideInclusion]
...
def nestedSupportRestrictionShortComplex (F : Sheaf AddCommGrpCat.{u} X) :
  ...
  ShortComplex.mk ((sectionsSupportedOutsideMap X h).app F)
    ((toSheafSectionsBetweenOpens X h).app F)
    (NatTrans.congr_app (sectionsSupportedOutsideMap_toBetween X h) F)
...
lemma nestedSupportRestrictionSectionsShortComplex_shortExact (W : Opens X)
    ...
    ((nestedSupportRestrictionShortComplex X h F).map Γ_[W]).ShortExact := by
  rw [nestedSupportRestrictionShortComplex_eq]
  have : Epi (Γ_[W].map ((toOpenRestrictionPushforward X U).app F)) :=
...
def nestedSupportRestrictionComplexShortComplex
  ...
  ShortComplex.mk
    (((sectionsSupportedOutsideMap X h).mapHomologicalComplex (.up ℤ)).app K)
    (((toSheafSectionsBetweenOpens X h).mapHomologicalComplex (.up ℤ)).app K)
    (by ext n; exact NatTrans.congr_app (sectionsSupportedOutsideMap_toBetween X h) (K.X n))
...
def nestedSupportRestrictionSectionsComplexShortComplex (W : Opens X)
  ...
  (nestedSupportRestrictionComplexShortComplex X h K).map
    (Γ_[W].mapHomologicalComplex (.up ℤ))
