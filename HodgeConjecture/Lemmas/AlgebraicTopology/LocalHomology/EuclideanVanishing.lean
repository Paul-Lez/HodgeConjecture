...
theorem standardPuncturedHomology_isZero_of_lt (d k : ℕ)
    (hk : k ≠ 0) (hlt : k + 1 < d) :
    IsZero (Homology ℚ (puncturedPair ℝ d).snd k) := by
...
theorem standardLocalHomology_isZero_of_dimension_lt (d k : ℕ) (hk : d < k) :
  ...
  exact (relativeSingular_homology_exact_relative (puncturedPair ℝ d)
...
theorem puncturedPair_nonempty (d : ℕ) (hd : 0 < d) :
    Nonempty (puncturedPair ℝ d).snd := by
  refine ⟨⟨fun _ ↦ 1, ?_⟩⟩
  intro h
...
theorem puncturedPair_pathConnectedSpace (d : ℕ) (hd : 1 < d) :
    PathConnectedSpace (puncturedPair ℝ d).snd := by
...
theorem standardLocalHomology_zero_isZero (d : ℕ) (hd : 0 < d) :
    IsZero (RelativeHomology ℚ (puncturedPair ℝ d) 0) := by
  let : Nonempty (puncturedPair ℝ d).snd := puncturedPair_nonempty d hd
  let : PathConnectedSpace (puncturedPair ℝ d).fst :=
    inferInstanceAs (PathConnectedSpace (Fin d → ℝ))
  exact relativeHomology_zero_isZero_of_pathConnected (puncturedPair ℝ d)

/-- Local degree-one homology vanishes in real dimension greater than one. -/
theorem standardLocalHomology_one_isZero (d : ℕ) (hd : 1 < d) :
    IsZero (RelativeHomology ℚ (puncturedPair ℝ d) 1) := by
  let : PathConnectedSpace (puncturedPair ℝ d).snd :=
    puncturedPair_pathConnectedSpace d hd
  let : PathConnectedSpace (puncturedPair ℝ d).fst :=
    inferInstanceAs (PathConnectedSpace (Fin d → ℝ))
  apply HomologicalComplex.ExactAt.isZero_homology
  refine (relativeChainShortComplex_shortExact ℚ (puncturedPair ℝ d)).exactAt_X₃ 1 ?_ ?_
  ...
  · intro j hj
    ...
    let : IsIso (HomologicalComplex.homologyMap
        ((chainPairFunctor ℚ).obj (puncturedPair ℝ d)).hom 0) :=
      TopCat.singularHomologyMap_zero_isIso
        (X := (puncturedPair ℝ d).snd) (Y := (puncturedPair ℝ d).fst)
        (puncturedPair ℝ d).map (ModuleCat.of ℚ ℚ)
...
theorem standardLocalHomology_isZero_of_ne (d k : ℕ) (hk : k ≠ d) :
  ...
  · rcases k with _ | _ | n
    ...
    · exact
        (relativeSingular_homology_exact_relative (puncturedPair ℝ d)
