...
noncomputable def pushforward
    (h : ∀ y, D.pushforwardCycle y ≠ 0 → coheight y = p) : codimensionCycleSubgroup X p :=
...
noncomputable def rationalEquivalenceSubgroup (X : Scheme.{u}) (p : ℕ) :
    AddSubgroup (codimensionCycleSubgroup X p) :=
...
noncomputable def mk (c : codimensionCycleSubgroup X p) : ChowGroup X p :=
  QuotientAddGroup.mk c

instance codimensionCycle_subsingleton_of_isEmpty [IsEmpty X] :
    Subsingleton (codimensionCycleSubgroup X p) where
  allEq a b := codimensionCycleSubgroup.ext (a := a) (b := b) fun x ↦ isEmptyElim x
...
instance subsingleton_of_isEmpty [IsEmpty X] : Subsingleton (ChowGroup X p) :=
  (QuotientAddGroup.subsingleton_iff).2 <| by
    ...
    rw [Subsingleton.elim a (0 : codimensionCycleSubgroup X p)]
...
noncomputable def codimensionZeroEquiv (X : Scheme.{u}) :
    ChowGroup X 0 ≃+ codimensionCycleSubgroup X 0 :=
...
noncomputable def integralEquiv (X : Scheme.{u}) [IsIntegral X] : ChowGroup X 0 ≃+ ℤ :=
  (codimensionZeroEquiv X).trans codimensionCycleSubgroup.integralEquiv
...
noncomputable def specFieldEquiv (K : Type u) [Field K] :
  ...
  (codimensionZeroEquiv (Spec ↧K)).trans (codimensionCycleSubgroup.specFieldEquiv K)
