...
/-- Let `R` be a commutative ring and `A` a commutative `R`-algebra. This is the exterior algebra
`⋀_A Ω_{A/R}` of the module of Kähler differentials. Its degree-`p` part is the module of
relative differential `p`-forms, generated by expressions `a₀ da₁ ∧ ⋯ ∧ daₚ`. -/
...
exterior algebra of Kähler differentials. The exterior derivative is the `R`-linear map `d : E →
E` determined by `d(a) = da`, `d(da) = 0`, and `d(ωη) = dω η + (-1)^p ω dη` for `ω` of degree
`p`. It raises degree by one. -/
...
/-- Let `R` be a commutative ring and `A` a commutative `R`-algebra. The relative differential
`p`-forms form the module `⋀^p_A Ω_{A/R}`, the `p`-th exterior power of the module of Kähler
differentials. Its elements are sums of expressions `a₀ da₁ ∧ ⋯ ∧ daₚ`. -/
abbrev Form (p : ℕ) : Type u := ↥(⋀[A]^p Ω[A⁄R])

/-- Let `R` be a commutative ring and `A` a commutative `R`-algebra. This `R`-linear exterior
derivative maps relative differential `p`-forms to `(p+1)`-forms. On generators it sends `a₀ da₁
∧ ⋯ ∧ daₚ` to `da₀ ∧ da₁ ∧ ⋯ ∧ daₚ`. -/
...
/-- Let `R` be a commutative ring and `A` a commutative `R`-algebra. For a family `v₀, …, v_{p-1}`
in `A`, this is the relative differential form `dv₀ ∧ ⋯ ∧ dv_{p-1}`. The empty wedge, for `p =
0`, is the constant zero-form `1`. -/
...
/-- Let `R` be a commutative ring and `A` a commutative `R`-algebra. For `a₀ ∈ A` and a family `v₀,
…, v_{p-1}` in `A`, this constructs the relative differential `p`-form `a₀ dv₀ ∧ ⋯ ∧ dv_{p-1}`
in `⋀^p_A Ω_{A/R}`. -/
...
/-- Let `R` be a commutative ring and `A` a commutative `R`-algebra. This `R`-linear map sends `a ∈
A` to the corresponding degree-zero relative differential form in `⋀^0_A Ω_{A/R}`. -/
...
/-- Let `R` be a commutative ring and `A` a commutative `R`-algebra. This `R`-linear map sends `r ∈
R` to the zero-form given by its image in `A` under the algebra structure map. -/
...
/-- Let `R → A → B` be a tower of commutative algebras. This `A`-algebra homomorphism `⋀_A Ω_{A/R} →
⋀_B Ω_{B/R}` sends `a` to its image in `B` and `da` to the differential of that image. -/
...
/-- Let `R → A → B` be a tower of commutative algebras. For a natural number `p`, this `R`-linear
map from relative `p`-forms of `A` to those of `B` applies `A → B` to each coefficient and each
function appearing in `a₀ da₁ ∧ ⋯ ∧ daₚ`. -/
...
/-- Let `A` and `B` be commutative algebras over a commutative ring `R`, and let `f : A → B` be an
`R`-algebra homomorphism. This induced map on relative `p`-forms sends `a₀ da₁ ∧ ⋯ ∧ daₚ` to
`f(a₀) d(f(a₁)) ∧ ⋯ ∧ d(f(aₚ))`. -/
