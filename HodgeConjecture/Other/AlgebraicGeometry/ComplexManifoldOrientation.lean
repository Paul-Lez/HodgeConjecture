...
public import HodgeConjecture.Other.Geometry.Manifold.Orientation
public import HodgeConjecture.Other.LinearAlgebra.ComplexOrientation
...
theorem isRealManifold_one [SmoothOfRelativeDimension d structureMap] :
    IsManifold 𝓘(ℝ, Fin d → ℂ) 1 (ComplexPoint X structureMap) := by
  apply isManifold_of_contDiffOn
  intro e e' he he'
  obtain ⟨z, rfl⟩ := he
  obtain ⟨z', rfl⟩ := he'
  have h := (contDiffOn_localChart_transition structureMap d z z').restrict_scalars ℝ
...
local instance complexManifoldForTangentComparison
    [SmoothOfRelativeDimension d structureMap] :
    IsManifold 𝓘(ℂ, Fin d → ℂ) ω (ComplexPoint X structureMap) :=
  isManifold_omega structureMap d

local instance realManifoldForTangentComparison
    [SmoothOfRelativeDimension d structureMap] :
    IsManifold 𝓘(ℝ, Fin d → ℂ) 1 (ComplexPoint X structureMap) :=
  isRealManifold_one structureMap d
...
lemma realTangentCoordChange_eq_restrictScalars
    [SmoothOfRelativeDimension d structureMap]
    (x y z : ComplexPoint X structureMap)
    (hx : z ∈ (localChart structureMap d x).source)
    (hy : z ∈ (localChart structureMap d y).source) :
  ...
  have hoverlapR : z ∈ (extChartAt 𝓘(ℝ, Fin d → ℂ) x).source ∩
    ...
    change z ∈ (localChart structureMap d x).source ∩
      (localChart structureMap d y).source
    exact ⟨hx, hy⟩
  ...
  calc
    ...
    _ = ((tangentCoordChange 𝓘(ℂ, Fin d → ℂ) x y z).restrictScalars ℝ).toLinearMap :=
      congrArg ContinuousLinearMap.toLinearMap
        (realTangentCoordChange_eq_restrictScalars structureMap d x y z hx hy)
...
def orientationLift [SmoothOfRelativeDimension d structureMap] :
    Manifold.OrientationLift 𝓘(ℝ, Fin d → ℂ) (ComplexPoint X structureMap) (Fin (d * 2)) where
  ...
  compatible x y z hx hy := by
    ...
    exact (Orientation.map_eq_iff_det_pos (Complex.piOrientation d)
      (tangentCoordChangeEquiv 𝓘(ℝ, Fin d → ℂ) x y z) Fact.out).2
        (realTangentCoordChangeEquiv_det_pos structureMap d x y z hx hy)

/-- The complex orientation on a smooth complex analytification. -/
def manifoldOrientation [SmoothOfRelativeDimension d structureMap] :
    Manifold.Orientation 𝓘(ℝ, Fin d → ℂ) (ComplexPoint X structureMap) (Fin (d * 2)) :=
  Manifold.Orientation.mk (orientationLift structureMap d)

/-- Smooth complex analytifications carry their canonical complex orientation as a real
manifold. -/
instance orientedManifold [SmoothOfRelativeDimension d structureMap] :
    Manifold.OrientedManifold 𝓘(ℝ, Fin d → ℂ)
      (ComplexPoint X structureMap) (Fin (d * 2)) where
  manifoldOrientation := manifoldOrientation structureMap d
