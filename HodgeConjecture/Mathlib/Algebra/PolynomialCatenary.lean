...
essentially character for character, the `equivT` section (lines 58-170) of Mathlib's
`Mathlib/RingTheory/NoetherNormalization.lean`, added to Mathlib in
`leanprover-community/mathlib4#18247`, whose copyright header reads "Copyright (c) 2025 Sihan Su.
...
`private`, rewrites or drops the docstrings, and renames two identifiers: `T_leadingcoeff_isUnit`
to `T_leadingCoeff_isUnit`, and the local hypothesis `coeff` to `coeff_eq`. It exists only because
those declarations are `private` upstream and hence cannot be imported; of them only `Triangular.T`
and `Triangular.T_leadingCoeff_isUnit` are used below, in
`MvPolynomial.height_add_ringKrullDim_quotient_eq_fin`. Mathlib pull request
`leanprover-community/mathlib4#43700` makes those two declarations public; once it lands and the
