...
A point of an integral scheme has coheight zero exactly when it is the generic point.
-/
...
lemma eq_genericPoint_of_coheight_zero {X : Scheme} [IsIntegral X] (x : X)
    (hx : Order.coheight x = 0) : x = genericPoint X := by
