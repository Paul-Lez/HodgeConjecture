...
`OriginalChernAmbientRawSection.lean` applies the local raw winding identity and
identifies the actual ambient Chern section with the ambient raw winding section.
The global assembly must still combine this equality with its supported normalization.
...
`CohomologySectionShiftOnOpen.lean` identifies the evaluated shift on every open
...
`CartierOriginalChernFrame.lean` supplies the actual regular frame and Cartier unit
on the overlap with the good complement, including their projection and frame-difference
identities. The comparison of this winding value with the original global supported class remains.
...
`ChernRelativeSupportGenericUnit.lean` specializes this to the Cartier unit on the
constructed generic chart, enlarging its component support to the bad locus.
