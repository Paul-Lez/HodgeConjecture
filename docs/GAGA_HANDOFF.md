...
[PR41](https://github.com/Paul-Lez/HodgeConjecture/pull/41) contains the GAGA development and is stacked on [PR9](https://github.com/Paul-Lez/HodgeConjecture/pull/9), which supplies the shared analytification interfaces and the separate divisor–Chern comparison. The general algebraization statement is in `GAGAStatement.lean`; `GAGAtoLefschetz.lean` is its only specialization to the Lefschetz reduction. The GAGA audit is `scripts/gaga_axiom_audit.lean`.

The split preserves the work in progress; it does not prove GAGA. The divisor–Chern calculation on PR9 takes an algebraic line bundle as input and does not assume GAGA.
...
`GAGAStatement.lean`; its proof remains open. The projective-space twist reduction is
substantially further along than the earlier dated sections below: `twistChartFramesProj`,
`twistRankOneAlgebraizesNonneg`, `twistRankOneAlgebraizesNeg`, and
`twistRelationsAlgebraizeProj` are proved. The remaining GAGA inputs are `AnalyticSerreGeneration`,
`AnalyticTwistPresentationKernelsFinite`, and
`AlgebraicTwistCokernelsReflectInvertibility` on projective space, together with the analytic
pushforward/compatibility and descent argument from a projective subvariety to its ambient
projective space. The dimension-zero and subsingleton cases are already handled in
`GAGALineBundles.lean`.
...
In [`Other/AlgebraicGeometry/GAGAStatement.lean`](../Other/AlgebraicGeometry/GAGAStatement.lean):
...
`Classical.choice`, `Quot.sound`. Add it to `scripts/gaga_axiom_audit.lean`.
`hasAlgebraicModel_of_analyticLineBundlesAlgebraize` in
[`Other/AlgebraicGeometry/GAGAtoLefschetz.lean`](../Other/AlgebraicGeometry/GAGAtoLefschetz.lean)
turns it into the form used by
...
- Topology for projective `X`: compactness `complexPoint_compactSpace` and paracompactness of
  every open `openParacompactSpace` are supplied by the shared projective analytification
  infrastructure, together with second countability.
...
- `Other/AlgebraicGeometry/ComplexPoint/ProjectiveConnected.lean` now also proves that complex
...
These declarations are included in `scripts/gaga_axiom_audit.lean` and use only `propext`,
...
proves, with no `sorry` and no new axiom (checked in `scripts/gaga_axiom_audit.lean`), the
...
*(P1) No standard affine chart of `ℙᴺ(ℂ)^an`.* The earlier assessment in
`ProjectiveAnalytificationHomeomorph.lean` had the
...
Two further files, both `sorry`-free and axiom-clean (added to `scripts/gaga_axiom_audit.lean`).
...
(all in `scripts/gaga_axiom_audit.lean`).  Together they supply the
...
- Two finer stepping stones for the first one are stated in the same file:
  * **`ChartRatioEvaluation N n`** — the chart-coordinate formula
    ...
    `awayCoordinateEvaluation_mk` / `chartIntegralProj` of
    `ProjectiveAnalytificationHomeomorph.lean` and the chart-evaluation modules,
