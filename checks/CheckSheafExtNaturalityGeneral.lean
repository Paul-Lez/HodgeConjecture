...
theorem check_sheafSingleShiftIso_naturality
...
private theorem check_constantExt_comp_mk₀_smallHom
...
theorem check_sheafExtHypercohomologyEquiv_naturality
    {F G : AnalyticAdditiveSheaf X} (s : F ⟶ G) (j : ℤ) (n : ℕ)
    (a : Abelian.Ext.{1} (constantIntegerSheaf X) F n) :
  ...
  rw [check_constantExt_comp_mk₀_smallHom]
  simp only [SmallHom.comp_assoc, SmallHom.mk_comp_mk]
  congr 2
  rw [check_sheafSingleShiftIso_naturality X s j n]
