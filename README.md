...
structures. Thirteen stand for mathematics Mathlib currently lacks, and results there taking one
as an argument hold conditionally on it; the two dualizing-complex structures carry no predicate
and are trivially inhabited (issue #100). In maximal codimension four of the fifteen are
constructed from the one-point calculation. `Other/AlgebraicGeometry/BorelMooreConditional.lean`
lists all fifteen with the construction each one waits on.
