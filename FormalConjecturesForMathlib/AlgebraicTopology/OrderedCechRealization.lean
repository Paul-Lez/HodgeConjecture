/-
Copyright 2026 Chris Birkbeck and The Formal Conjectures Authors.
...
public import FormalConjecturesForMathlib.AlgebraicTopology.OrderedCechTupleAlgebra
...
commit `5701e7211a1129650185feda1660fb44fbde9aef`.
...
open CategoryTheory CategoryTheory.Limits Finsupp AlgebraicTopology.OrderedCechTuple
...
public theorem faceOrZero_comp_faceOrZero {n m k : ℕ} (a : Fin (n + 1) → ι) (b : Fin (m + 1) → ι)
  ...
  unfold faceOrZero
  rw [dif_pos ((tupleSupport_subset_iff a b).2 hba), dif_pos ((tupleSupport_subset_iff b c).2 hcb),
    dif_pos ((tupleSupport_subset_iff a c).2 (hcb.trans hba)), M.face_comp]

public theorem faceOrZero_self {n : ℕ} (a : Fin (n + 1) → ι) : M.faceOrZero a a = 𝟙 _ := by
  unfold faceOrZero
  rw [dif_pos (Finset.Subset.refl _), M.face_id]
