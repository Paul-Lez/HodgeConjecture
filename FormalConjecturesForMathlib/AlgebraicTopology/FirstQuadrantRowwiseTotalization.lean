...
public import FormalConjecturesForMathlib.AlgebraicTopology.FirstQuadrantColumnFiltrationShortExact
...
`b200b3fa92c64b73f3f212026f191f85364f05e3`.  The generic horizontal-row definitions are
spelled out here so this module has no dependency on the paper-specific boundary-seven
development.
...
public theorem firstQuadrantColumnPrefixTotalToOriginal_hom_inv
  ...
  have he : e.hom.f q ≫ e.inv.f q = 𝟙 _ :=
    HomologicalComplex.congr_hom e.hom_inv_id q
  have hinv (h : p ≤ N) :
      (firstQuadrantColumnPrefixXIso K N p h).inv.f q = e.inv.f q := by
    congr
  rw [hinv, ← Category.assoc, he, Category.id_comp, Category.comp_id]
...
public theorem firstQuadrantColumnPrefixTotalToOriginal_inv_hom
  ...
  let e := firstQuadrantColumnPrefixXIso K N p hp
  have he : e.inv.f q ≫ e.hom.f q = 𝟙 _ :=
    HomologicalComplex.congr_hom e.inv_hom_id q
  have hinv (h : p ≤ N) :
      (firstQuadrantColumnPrefixXIso K N p h).inv.f q = e.inv.f q := by
    congr
  rw [hinv, ← Category.assoc, he, Category.id_comp, Category.comp_id]
...
public theorem firstQuadrantSingleColumnTotalXHom_inv
  ...
  · subst r
    ...
    rw [HomologicalComplex₂.ι_totalDesc_assoc]
    let e := firstQuadrantSingleColumnXIso K p
    have he : e.hom.f q ≫ e.inv.f q = 𝟙 _ :=
      HomologicalComplex.congr_hom e.hom_inv_id q
    simp only [dif_pos]
