...
models is done in `FormalConjecturesForMathlib.AlgebraicTopology.OrderedCechRealization`.
-/
...
public theorem firstRepeatHomotopy_single_of_eq {a : Fin (n + 2) → ι} (h : a 0 = a 1) :
  ...
  simp only [firstRepeatHomotopy, Finsupp.linearCombination_single, one_smul, if_pos h]
...
public theorem firstRepeatHomotopy_single_of_ne {a : Fin (n + 2) → ι} (h : a 0 ≠ a 1) :
  ...
  simp only [firstRepeatHomotopy, Finsupp.linearCombination_single, one_smul, if_neg h]
