...
/-- A `K`-point of a scheme over `K`. -/
abbrev KPoint (K : Type) [Field K] (X : Scheme) (structureMap : X ⟶ Spec (.of K)) :=
  {z : Spec (.of K) ⟶ X // z ≫ structureMap = 𝟙 _}

namespace KPoint

variable {K : Type} [Field K] {X : Scheme} {structureMap : X ⟶ Spec (.of K)}

/-- The scheme point and residue-field embedding corresponding to a `K`-point. -/
noncomputable abbrev residueData (z : KPoint K X structureMap) :
    Σ x : X, X.residueField x ⟶ .of K :=
  Scheme.SpecToEquivOfField K X z.1

/-- The underlying point of a `K`-point. -/
noncomputable abbrev underlying (z : KPoint K X structureMap) : X := z.residueData.1

/-- The `K`-points whose underlying scheme point lies in `U`. -/
noncomputable abbrev overOpen (U : X.Opens) : Set (KPoint K X structureMap) :=
  {z | z.underlying ∈ U}

/-- Evaluation of a local regular function at a `K`-point, extended by zero outside its domain. -/
noncomputable abbrev evaluate (U : X.Opens) (s : Γ(X, U))
    (z : KPoint K X structureMap) : K := by
  classical
  exact if hz : z.residueData.1 ∈ U then
      z.residueData.2 (X.evaluation U z.residueData.1 hz s)
    else 0

/-- The subbasis for the topology on the `K`-points of a scheme over a topological field `K`.
It is generated locally by evaluation of regular functions. -/
noncomputable def topologySubbasis [TopologicalSpace K] :
    Set (Set (KPoint K X structureMap)) :=
  {W | ∃ (U : X.Opens) (s : Γ(X, U)) (V : Set K), IsOpen V ∧
    W = overOpen U ∩ evaluate U s ⁻¹' V}

/-- The topology on the `K`-points of a scheme over a topological field `K`, generated by
evaluation of local regular functions. In particular this applies when `K` is a normed field. -/
@[instance_reducible]
noncomputable def topology [TopologicalSpace K] : TopologicalSpace (KPoint K X structureMap) :=
  TopologicalSpace.generateFrom topologySubbasis

end KPoint

/-- A complex point of `X` over its given structure morphism to `Spec ℂ`. -/
abbrev ComplexPoint (X : Scheme) (structureMap : X ⟶ Spec (.of ℂ)) :=
  KPoint ℂ X structureMap
...
/-- The complex-point topology is the general topology on `K`-points, specialized to `K = ℂ`. -/
lemma analyticTopology_eq_kPointTopology :
    analyticTopology (X := X) (structureMap := structureMap) =
      KPoint.topology (K := ℂ) (X := X) (structureMap := structureMap) := by
  congr 1
